        ...
        try {
            if ($isEdit) {
                ...
                $stmt->execute([
                    ...
                    $alumno['fecha_nacimiento'], $alumno['telefono'], 
                    $alumno['direccion'], $alumno['genero'], $_GET['id']
                ]);
                $_SESSION['message'] = 'Alumno actualizado con éxito';
            } else {
                $stmt = $conn->prepare("
                    INSERT INTO alumnos (cedula, nombre, apellido, fecha_nacimiento, 
                                       telefono, direccion, genero)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                ");
                $stmt->execute([
                    $alumno['cedula'], $alumno['nombre'], $alumno['apellido'], 
                    $alumno['fecha_nacimiento'], $alumno['telefono'], 
                    $alumno['direccion'], $alumno['genero']
                ]);
                $_SESSION['message'] = 'Alumno creado con éxito';
            }
            header('Location: ?module=alumnos');
            exit;
        } catch(PDOException $e) {
            if ($e->errorInfo[1] == 1062) { // Error de duplicado
                $error = 'Ya existe un alumno con esa cédula';
            } else {
...
<div class="card">
    <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
            <h5 class="mb-0"><?php echo $isEdit ? 'Editar' : 'Nuevo' ?> Alumno</h5>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control" id="cedula" name="cedula" 
                           value="<?php echo htmlspecialchars($alumno['cedula']); ?>" required>
                </div>
                ...
                <div class="col-12 mb-3">
                    ...
                    <textarea class="form-control" id="direccion" name="direccion" rows="3"><?php echo htmlspecialchars($alumno['direccion']); ?></textarea>
                </div>
            </div>
